/**
 * app-probe5.mjs — dump wdkb menu models (name/modelName/url/controls) to map the
 * schedule & course query endpoints. Values here are app structure, not personal.
 */
...
async function main() {
  ...
  try {
    ...
    // capture the app landing POST body for xskcb.do
    let body = '';
    page.on('request', (req) => {
...
main().catch((e) => {
  process.stderr.write(`app-probe5 failed: ${e.message}\n`);
  process.exit(1);
});
